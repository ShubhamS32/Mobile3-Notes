...
###Sms Demo
* In android there are two ways to send a message from your application. Here i will be using the in-build feature provided by the android.
* These are the steps you need to follow : 
	* **Step 1:**Give permission in the manifest file. (AndroidManifest.xml)
	* **Step 2:**Create the layout with just one button.(activity_main.xml)
	* **Step 3:**Write a intent in the main class. (MainActivity.java)

###Scan Beacon Device Demo
* This application show how to use your android set as a beacon scanner. You can find this application in the android sdk version 4.4 samples projects.
* To get started with the application you need to follow these steps.
	* Give the permission in the manifest file along with that declare an activity and service.(AndroidManifest.xml)
	* Create a layout for the main screen to show number of available devices. (listitem_device.xml)
	* Create a layout for the beacon characteristics. (gatt_services_characteristics.xml) 
	* Create a layout to handle the action bar. (actionbar_indeterminate_progress.xml)
	* Write a main class. (DeviceScanActivity.java)
